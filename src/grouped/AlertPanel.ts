...
export class AlertPanel extends UIPanelBase {
	//} extends UIMain{
	...
	renderButtons() {
		return html`
			${this.alertType == AlertType.alert
				? html`
						<domui-button @callback=${this.onYes}
							>${this.buttonName('confirm')}</domui-button
						>
					`
				: ''}
			${this.alertType == AlertType.confirm
				? html`
						<domui-hori-container>
							<domui-button @callback=${this.onYes}
								>${this.buttonName('yes')}</domui-button
							>
							<domui-button @callback=${this.onNo}
					...
					`
				: ''}
			${this.alertType == AlertType.input
				? html`
						<input type="text" />
						<domui-hori-container>
							<domui-button @callback=${this.onYes}
    ...
    */
	...
	onCancel() {
		dispatchEvent(
			new CustomEvent('Response', { detail: { cancel: true } })
		);
		this.close();
	}
