...
export default class TabContainer extends Container {
  ...
  updated(_changedProperties: Map<PropertyKey, unknown>): void {
    ...
    if (
      _changedProperties.has("currentTabIndex") ||
      _changedProperties.has("slotElements")
    ) {
      setTimeout(() => {
  ...
  renderTabBtn(title: string, id: number) {
    if (this.tabSelectable) {
      return html`
        <domui-button
          @callback=${() => this.switchTab(id)}
          ${this.currentTabIndex == id ? "active selected" : ""}
          class="tabBtn"
        >
          ${title}
        </domui-button>
    ...
    return html`
      <div
        ...
        class="tabBtn ${this.currentTabIndex == id ? "active" : ""}"
      >
        ${title}
      </div>
    `;
