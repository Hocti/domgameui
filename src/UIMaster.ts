...
export default class UIMaster extends EventTarget {
	...
	private constructor(_div: HTMLDivElement) {
		...
		this.rootElement = this._div.getElementsByTagName(
			'root-screen'
		)[0] as UI;
		...
		for (const name of layerNames) {
			this.layers[name] = this.rootElement.querySelector(
				`domui-layer[name="${name}"]`
			)! as UI;
		}

		ControlWrap.getInstance().addListener(
			'changeLastInputIndex',
			this.changeInputType
		);

		this.rootElement.addEventListener(
			'mousedown',
			this.mouseChange.bind(this)
		);
		this.rootElement.addEventListener(
			'mousemove',
			this.mouseChange.bind(this)
	...
	public setSize(w: number = 1280, h: number = 720, scale: number = 1): void {
		this.fixSize = true;
		this.innerSetSize(
			Math.max(this.minWidth, w),
			Math.max(this.minHeight, h),
			scale
		);
	}
	...
	private innerSetSize(
		...
		if (this.rootElement) {
			...
			this.rootElement.style.setProperty(
				'--rootS',
				Math.min(newHeight, newWidth) / 100 + 'px'
			);
			this.rootElement.style.setProperty(
				'--rootL',
				Math.max(newHeight, newWidth) / 100 + 'px'
			);
			this.rootElement.style.setProperty(
				'--rootFontsize',
				this._fontsize + 'px'
			...
			this.rootElement.style.left =
				Math.round((this._div.clientWidth - newWidth * scale) / 2) +
				'px';
			this.rootElement.style.top =
				Math.round((this._div.clientHeight - newHeight * scale) / 2) +
				'px';
		}
	...
	public async closedPanel(panel: UIPanel) {
		...
		if (id == this.activePanels.length - 1) {
			...
			this.activePanels = utils.removeElementWithIndex(
				this.activePanels,
				id
			);
		}
	...
	private setActiveChild() {
		setTimeout(() => {
			...
			if (
				!this.activeChild &&
				this.activeUI.cursorChild &&
				isUIParent(this.activeUI.cursorChild)
			) {
				let currChild: UIChild = this.activeUI.cursorChild;
				while (
					(currChild as unknown as UIParent).cursorChild &&
					isUIParent((currChild as unknown as UIParent).cursorChild)
				) {
	...
	public setActiveComponent(com?: UISelectable): boolean {
		//success
	...
	public removeIfActive(com: UIChild) {
		...
		if (isUIParent(com)) {
			(com as unknown as UIParent)
				.getChildren()
				.forEach((child: UIChild) => {
