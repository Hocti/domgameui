...
export interface UI extends LitElement {
	//BG
...
export interface UIParent extends UIInteractive {
	//UIRoot,Container
...
export interface UIChild extends UIInteractive {
	//button,Container
...
export interface UISelectable extends UIChild {
	//button
	active: boolean;
	lock: boolean;
...
export interface UIInput<T extends inputType> extends UISelectable {
	//checkbox,selector
	getVal(): T;
	setVal(val: T): void;
}
...
export function isUIChild(element: any): boolean {
	return (
		element && element.getParent && typeof element.getParent === 'function'
	);
}
export function isUIParent(element: any): boolean {
	return (
		...
		typeof element.captureAllInput === 'function'
	);
}
...
export function isUIPanel(element: any): boolean {
	return (
		isUIParentRoot(element) && typeof element.clickBG2Close === 'boolean'
	);
}
