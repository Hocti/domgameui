...
import { getParent, getRoot, setParentsCursorToMe } from './parentingUtils';
...
export function getSingletonUI<T extends UIBase>(classRef: new () => T): T {
	if (!cacheInstance.has(classRef.name)) {
		cacheInstance.set(
			classRef.name,
			(classRef as unknown as typeof UIBase).createInstance()
		);
	}
...
export abstract class UIBase extends LitElement implements UI {
	...
	static prepare(eleName?: string) {
		...
		const superElementName = (
			Object.getPrototypeOf(this) as unknown as typeof UIBase
		).eleName;
	...
	render() {
		if (
			this.constructor.hasOwnProperty('tempCss') ||
			(this.constructor as unknown as typeof UIBase).useTempCss
		) {
			return html`<style>
					${(this.constructor as unknown as typeof UIBase).tempCss}</style
				>${this.renderWrap(this.renderCore())}`;
		}
	...
	constructor() {
		...
		const observer = new IntersectionObserver(
			...
			{ threshold: 0.05 }
		); // Configure the observer to trigger when 10% visible
...
export abstract class UIChildBase extends UIInteractiveBase implements UIChild {
	//button,Container
