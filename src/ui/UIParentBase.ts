...
import { getUIChildren } from './parentingUtils';

import { UIInteractiveBase } from './SimpleBaseUI';

export default abstract class UIParentBase
	extends UIInteractiveBase
	implements UIParent
{
	uiChildren: UIChild[] = [];
    ...
    */
	...
	updated(_changedProperties: Map<PropertyKey, unknown>): void {
		...
		if (
			this.cursorChild &&
			this.uiChildren.indexOf(this.cursorChild) == -1
		) {
	...
	private autoFindChildCursor(
		...
		for (let i = 0, t = this.uiChildren.length; i < t; i++) {
			if (isUIParent(this.uiChildren[i])) {
				(this.uiChildren[i] as unknown as UIParent).autoFindCursor(
					selectFirst,
					autoActive
				);
			}
    ...
    */
	...
	renewSelectable(): boolean {
		if (this.uiChildren.length > 0) {
			for (let i = 0, t = this.uiChildren.length; i < t; i++) {
				if (
					isUISelectable(this.uiChildren[i]) &&
					(this.uiChildren[i] as UISelectable).unselectable === false
				) {
					this.cursorSelectable = true;
				} else if (isUIParent(this.uiChildren[i])) {
					const result = (
						this.uiChildren[i] as unknown as UIParent
					).renewSelectable();
	...
	private findAndActiveIfSelectable(
		element: UIChild
	): UISelectable | undefined {
		if (
			isUISelectable(element) &&
			(element as UISelectable).unselectable === false
		) {
			...
			const result = (
				element as unknown as UIParent
	...
	findAndActiveFirstSelectable(): UISelectable | undefined {
		...
		if (this.uiChildren.length > 0) {
			if (this.cursorIndex != -1 && this.uiChildren[this.cursorIndex]) {
				const result1 = this.findAndActiveIfSelectable(
					this.uiChildren[this.cursorIndex]
				);
				if (result1) {
					return result1;
				}
			...
			for (let i = 0, t = this.uiChildren.length; i < t; i++) {
				...
				const result2 = this.findAndActiveIfSelectable(
					this.uiChildren[i] as UIChild
				);
	...
	getNewCursor(add: number, fromIndex: number = -1): [boolean, number] {
		...
		if (this.loopCursor) {
			newIndex =
				((newIndex % this.uiChildren.length) + this.uiChildren.length) %
				this.uiChildren.length;
		} else if (newIndex < 0) {
	...
	captureInput(ip: Input): boolean {
		...
		if (
			((udlr.x !== 0 &&
				!(this.constructor as typeof UIParentBase).ignoreX) ||
				(udlr.y !== 0 &&
					!(this.constructor as typeof UIParentBase).ignoreY)) &&
...
export abstract class UIParentRootBase
	extends UIParentBase
	implements UIParentRoot
{
	//main
